...
class multifilter:
    # Определяем три статических метода-фильтра, которые будут использоваться по умолчанию
    def judge_half(pos, neg):
        return pos >= neg
    ...
    # Определяем конструктор класса, который принимает на вход итерируемый объект, список функций-фильтров, и выбранный
    # фильтр judge.
    def __init__(self, iterable, *funcs, judge=judge_any):
        self.iterator = iter(iterable)  # Создаем итератор для итерируемого объекта
        self.funcs = funcs  # Список функций-фильтров
        self.judge = judge  # Выбранный фильтр

    # Определяем метод __iter__, который будет возвращать итератор класса
    def __iter__(self):
        return self

    # Определяем метод __next__, который будет возвращать следующий отфильтрованный элемент итерируемого объекта
    def __next__(self):
        while (True):
            elem = next(self.iterator)  # Получаем следующий элемент итерируемого объекта
            pos, neg = 0, 0  # Инициализируем счетчики для подсчета количества элементов, прошедших/не прошедших фильтры
            for func in self.funcs:  # Проходим по списку функций-фильтров
                if func(elem):  # Если элемент проходит фильтр
                    pos += 1  # Увеличиваем счетчик прошедших фильтр
                else:
                    neg += 1  # Увеличиваем счетчик не прошедших фильтр

            # Если элемент прошел выбранный фильтр
            if self.judge(pos, neg):
                return elem  # Возвращаем элемент


# Определяем внешние функции-фильтры
def mul2(x):
    return x % 2 == 0
